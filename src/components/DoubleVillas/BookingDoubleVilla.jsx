...
export default function BookingDoubleVilla() {
  const { selectedDoubleVilla } = useDoubleVilla();
  const recaptchatRef = useRef(null);
    ...
    return (
      <div className="text-center py-20">
        <p className="text-slate-600">
          No has seleccionado ninguna Villa Doble.
        </p>
      </div>
    );
  }
  ...
  const handleSubmit = (e) => {
    ...
    const token = recaptchatRef.current.getValue();
    if (!token) {
      alert("Por favor verifica que no eres un robot.");
      return;
    }

    recaptchatRef.current.reset();
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white rounded-2xl shadow-md">
      <h2 className="text-2xl font-semibold text-slate-800 mb-4">
        Reservar {selectedDoubleVilla.name}
      </h2>

      <div className="space-y-2 mb-6">
        <p>
          <strong>Área:</strong> {selectedDoubleVilla.area}
        </p>
        <p>
          <strong>Precio:</strong> ${selectedDoubleVilla.price.toLocaleString()}
        </p>
      </div>
